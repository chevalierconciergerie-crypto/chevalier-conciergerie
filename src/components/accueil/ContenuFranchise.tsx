  ...
  return (
    <>
      <Revele className="chv-entree">
        <span className="chv-badge">{t("Le réseau Chevalier")}</span>
        <Titre className="chv-sous-titre">
          {t("Vous connaissez votre ville.")}
          <br />
          <em>{t("Nous connaissons le métier.")}</em>
        </Titre>
        <p className="chv-paragraphe" style={{ maxWidth: 640 }}>
          {t("Monter une conciergerie seul, c'est deux ans à apprendre par l'erreur : les propriétaires qu'on ne trouve pas, les prix qu'on fixe au hasard, les ménages qui s'enchaînent mal, le logiciel qu'on n'a pas. Nous avons fait ces deux ans. Le réseau existe pour que vous ne les refassiez pas.")}
        </p>
        <p className="chv-paragraphe" style={{ maxWidth: 640, marginTop: 16 }}>
          {t("Chevalier Conciergerie gère des logements à Avignon, Villeneuve-lès-Avignon et Les Angles, avec une note de 5,0 sur 5 sur Google. Ce n'est pas une méthode écrite pour être vendue : c'est celle que nous utilisons tous les jours, transmise telle quelle.")}
        </p>
      </Revele>

      {/* Ce qui est compris — liste numérotée à filets */}
      <Revele className="chv-entete-section" style={{ marginTop: "min(8vw, 88px)" }}>
        <span className="chv-badge">{t("Ce qui est compris")}</span>
        <h3 className="chv-sous-titre">
          {t("Tout ce qu'il faut")}
          <br />
          <em>{t("pour ouvrir, et pour tenir.")}</em>
        </h3>
      </Revele>
      ...
      <Revele className="chv-entete-section" style={{ marginTop: "min(8vw, 88px)" }}>
        <span className="chv-badge">{t("À qui ça s\'adresse")}</span>
        <h3 className="chv-sous-titre">
          {t("Trois façons d'arriver,")}
          <br />
          <em>{t("un seul métier à la sortie.")}</em>
        </h3>
      </Revele>
      ...
      <Revele className="chv-entete-section" delai={140} style={{ marginTop: "min(5vw, 44px)" }}>
        <p className="chv-paragraphe" style={{ maxWidth: 560, marginInline: "auto" }}>
          {t("Ce qui compte n'est pas le diplôme ni l'apport : c'est d'habiter la ville que vous voulez couvrir, et de décrocher quand un voyageur appelle à vingt-trois heures.")}
        </p>
      </Revele>

      {/* Le parcours, du premier appel à l'ouverture */}
      <Revele className="chv-entete-section" style={{ marginTop: "min(8vw, 88px)" }}>
        <span className="chv-badge">{t("Le parcours")}</span>
        <h3 className="chv-sous-titre">
          {t("Du premier appel")}
          <br />
          <em>{t("à votre première remise de clés.")}</em>
        </h3>
      </Revele>
      ...
      <Revele className="chv-entete-section" delai={120} style={{ marginTop: "min(6vw, 56px)" }}>
        <p className="chv-paragraphe" style={{ maxWidth: 520, marginInline: "auto", marginBottom: 24 }}>
          {t("Le reste — le droit d'entrée, la redevance, le contrat — se dit de vive voix, une fois que nous savons de quelle ville vous parlez.")}
        </p>
        <Link className="chv-pastille" to={LIEN_APPEL}>
          {t("Réserver un appel")}
        </Link>
